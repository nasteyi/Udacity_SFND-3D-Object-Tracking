    ...
    bool bWait)
{
    ...
    for(const auto& it1 : boundingBoxes)
    {
        ...
        for(const auto& it2 : it1.lidarPoints)
        {
            ...
            int y = static_cast<int>((-xw * imageSize.height /
                worldSize.height) + imageSize.height);

            int x = static_cast<int>((-yw * imageSize.width /
                worldSize.width) + imageSize.width / 2);
    ...
    const std::vector<cv::DMatch> &kptMatches)
{
    ...
    {
        sum += cv::norm(
            kptsCurr.at(it.trainIdx).pt - kptsPrev.at(it.queryIdx).pt);
    }
    ...
    for (
        auto it = boundingBox.kptMatches.begin();
...
void computeTTCLidar(
    const std::vector<LidarPoint> &lidarPointsPrev,
    ...
    for (const auto& prevBox : prevFrame.boundingBoxes)
    {
        ...
        for (const auto& currBox : currFrame.boundingBoxes)
        {
            for (const auto &match : matches)
            {
                ...
                {
                    const auto &currPt = currFrame.keypoints.at(
                        match.trainIdx).pt;
