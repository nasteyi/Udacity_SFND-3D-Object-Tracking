...
int main(int argc, const char *argv[])
{
    ...
    for(std::size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        float shrinkFactor = 0.1f; // shrinks each bounding box by the given percentage to avoid 3D object merging at the edges of an ROI
        clusterLidarWithROI(
            (dataBuffer.end()-1)->boundingBoxes,
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            matchBoundingBoxes(
                ...
                bbBestMatches,
                *(dataBuffer.end()-2),
                *(dataBuffer.end()-1)); // associate bounding boxes between current and previous frame using keypoint matches
            ...
            for(auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    if (bVis)
                    {
                        ...
                        cv::putText(visImg, oss.str(), cv::Point2f(80, 50), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0,0,255));
